import java.util.*;
public class Board implements IBoard, IMinimax {
    public Board() {
    ...
    public boolean isTerminal() {
        Board[] successors = getSuccessors('X');
        Board[] npsucc = getSuccessors('O');
    ...
    public Board[] getSuccessors(char symbol) {
        ...
        Board[] successors = new Board[stateList.size()];
        for(int i = 0; i < stateList.size(); ++i) {
            successors[i] = new Board(stateList.get(i));
        }
        return successors;
    }
    ...
    public static Board nextState; // Optimal next move.
    ...
    public int max_value_with_pruning(Object state, int alpha, int beta) {
        if(state == null || !(state instanceof Board)) throw new ClassCastException();
        Board curr_state = (Board) state;
        ...
        if( curr_state.isTerminal() ) {
            //if(firstIteration) nextState = null;
            return 0;
        }
        
        Board[] successors = curr_state.getSuccessors('X');
        for(Board s : successors){
            int min = min_value_with_pruning(s, alpha, beta);
            //alpha = min > alpha ? min : alpha;
            //if(firstIteration) nextState = s;
    ...
    public int min_value_with_pruning(Object state, int alpha, int beta) {
        ...
        int score = curr_state.getScore();
        if (score == 100) return score ;
        if (score == -100)return score ; // HERE
        if( curr_state.isTerminal() ) {
            //if(firstIteration) nextState = null;
            return 0;
        }
        
        Board[] successors = curr_state.getSuccessors('O');
        for(Board s : successors){
